...
function initInfoAttachmentButtons(){
    ...
    $("#btnUpload").click(function(){
        $.ajax({
            type: 'GET',
            url: contextPath + "/upload/addBond/"+$("#hidAttachmentBondCode").val(),
            cache: false,
...
function findInfoAttachmentData(pageOffset) {
    ...
    var dataJson = {
        ...
        offset: pageOffset,
        processType: 1,
        bondCode: $("#hidAttachmentBondCode").val(),
        stime: $("#stime").val(),
        etime: $("#etime").val(),
    ...
    $.ajax({
        url: contextPath + "/bond/findDocumentFileInfo",
        type:"post",
...
function auditUpload(objFile){
    ...
    layer.confirm(
        '您是否要确认通过项目负责人上传的材料？确认后您也可以进行"修改"和"删除"',
        {icon: 3, title:'上传材料审核确认'},
        function(index) {
            $.ajax({
