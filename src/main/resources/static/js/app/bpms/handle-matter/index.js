...
function initTable() {
    ...
    $.ajax({
        url: contextPath + "/handle-matter/findHandleMatters",
        type:"post",
...
function createTr(processInfo,matter) {
    ...
    var noticeFlag = processInfo.noticeFlag;
    if(noticeFlag==0 || noticeFlag==null){
        html +='<div class="td-div-row"><a class="glyphicon glyphicon-flag"  onclick="updateNoticeFlag(this,'+processInfo.processId+')"></a></div>'
    }else if(noticeFlag==1){
        html +='<div class="td-div-row"><a class="glyphicon glyphicon-flag flag-color" onclick="updateNoticeFlag(this,'+processInfo.processId+')"></a></div>'
    }

    html +=' <div class="td-div-row">'+convertOperate(processInfo.status,processInfo.processId,matter.id,matter.orderIndex)+'</div></td>' ;
    return html;
}
function convertOperate(status,pId,id,orderIndex) {
    var result = '';
    switch (status) {
        case 2:
            result = '<a onclick="handleMatterInfo('+id+','+orderIndex+','+pId+','+status+')">处理事项</a>';
            break;
        case 8:
            result = '<a onclick="handleMatterInfo('+id+','+orderIndex+','+pId+','+status+')">处理事项</a>';
            break;
...
function createTabSecond(processInfo) {
    ...
    if(processInfo.type==0){
        ...
        html +='<span class="company-font" onclick="companyInfo(this)">（发行人事项）</span>';
    }
...
function updateNoticeFlag(ob,processId) {
    var flag=0;
    if(!$(ob).hasClass("glyphicon glyphicon-flag flag-color")){
        flag=1;
    }
    $.ajax({
        url: contextPath + "handle-matter/update-notice-flag/"+processId+"/"+flag,
        type:"post",
        ...
        success:function(data){
            if(data.code==0) {
                if (flag == 0) {
                    $(ob).removeClass();
                    $(ob).addClass("glyphicon glyphicon-flag");
                } else if (flag == 1) {
                    $(ob).removeClass();
                    $(ob).addClass("glyphicon glyphicon-flag flag-color");
                }
            }else{
