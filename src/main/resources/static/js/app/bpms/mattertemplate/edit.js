/**
 * Created by Dealing076 on 2018/3/1.
 */
//@ sourceURL=edit.js
var tmpJson = {};
$(function () {
    ...
    $("#divInfoTemplate").css({"height": $(".layui-layer-page").height()-150 + "px", "overflow-x":"hidden","overflow-y":"auto"});
...
function initAddControl(){
    ...
    $("#editSave").click(function(){
        if(addValidate()){
            ...
            $.ajax({
                url: contextPath + "/mattertemplate/editSave",
                type:"post",
...
function addValidate(){
    ...
    if(radioTime==0){
        $(".divTime").each(function(index, item){
            var stime = trim($(item).find("input[name='remindDate1']").val());
            if(stime==""){
                layer.alert("保存失败：提醒时间不能为空", {icon: 1, time: 3000});
                isFlag = false;
                return false;
            }
            var etime = trim($(item).find("input[name='completeDate1']").val());
            if(etime==""){
