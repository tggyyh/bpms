...
function loadViewInfo(){
    ...
    if(orderIndex==0) {
        $.ajax(contextPath + "inspector-trace-matter/" + $("#hidTaskId").val()).success(function (data) {
            console.log("success");
            $("#divView").html("");
            $("#divView").append(data);
        });
    }else{
        $.ajax(contextPath + "inspector-trace-matter/right-line/" + $("#hidTaskId").val()).success(function (data) {
            console.log("success");
            $("#divView").html("");
            $("#divView").append(data);
        });
